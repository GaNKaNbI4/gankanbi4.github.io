    ...
    (() => {
        ...
        function showMoreFunc() {
            ...
            function getHeight(showMoreBlock, showMoreContent) {
                let hiddenHeight = 0;
                let gapFirstEl = document.querySelector("[data-gap-first]");
                let gapSecondEl = document.querySelector("[data-gap-second]");
                ...
                if ("items" === showMoreType) {
                    ...
                    if (window.matchMedia("(max-width: 31.25rem)").matches) showMoreTypeValue = 4;
                    const showMoreItems = showMoreContent.children;
                    for (let index = 1; index < showMoreItems.length; index++) {
                        const showMoreItem = showMoreItems[index - 1];
                        let gap = gapSecondEl.getBoundingClientRect().x - gapFirstEl.getBoundingClientRect().x - gapFirstEl.offsetWidth;
                        if (window.matchMedia("(max-width: 25rem)").matches) hiddenHeight += showMoreItem.offsetHeight + gap; else hiddenHeight += showMoreItem.offsetHeight + gap - 5;
        ...
        class ChiefSlider {
            constructor(selector, config) {
                const $root = "string" === typeof selector ? document.querySelector(selector) : selector;
                this._$root = $root;
                ...
                if (this._config.loop) {
                    ...
                    const translate = 100 * -$itemList.length;
                    $itemList[count].dataset.order = -1;
                    $itemList[count].dataset.translate = 100 * -$itemList.length;
            ...
            _addEventListener() {
                ...
                function onSwipeStart(e) {
                    this._autoplay("stop");
                    const event = 0 === e.type.search("touch") ? e.touches[0] : e;
                    this._swipeStartPos = event.clientX;
                    this._hasSwipeState = true;
                }
                function onSwipeEnd(e) {
                    if (!this._hasSwipeState) return;
                    const event = 0 === e.type.search("touch") ? e.changedTouches[0] : e;
                ...
                function onVisibilityChange() {
                    if ("hidden" === document.visibilityState) this._autoplay("stop"); else if ("visible" === document.visibilityState) if (this._config.loop) this._autoplay();
                }
                $root.addEventListener("click", onClick.bind(this));
                $root.addEventListener("mouseenter", onMouseEnter.bind(this));
                $root.addEventListener("mouseleave", onMouseLeave.bind(this));
                if (config.refresh) window.addEventListener("resize", onResize.bind(this));
                if (config.loop) {
            ...
            _setActiveClass() {
                ...
                for (let i = 0, length = $itemList.length; i < length; i++) {
                    ...
                    if (activeItems.indexOf(index) > -1) $item.classList.add(CLASS_ITEM_ACTIVE); else $item.classList.remove(CLASS_ITEM_ACTIVE);
                }
            }
            _updateIndicators() {
                ...
                for (let index = 0, length = $itemList.length; index < length; index++) {
                    ...
                    if ($item.classList.contains(CLASS_ITEM_ACTIVE)) $indicatorList[index].classList.add(CLASS_INDICATOR_ACTIVE); else $indicatorList[index].classList.remove(CLASS_INDICATOR_ACTIVE);
                }
            }
            _move() {
                const step = "next" === this._direction ? -this._transformStep : this._transformStep;
                ...
                if (!this._config.loop) {
                    ...
                    transform = Math.round(10 * transform) / 10;
                    if (transform < -endTransformValue || transform > 0) return;
                    if (this._$controlPrev) {
                        this._$controlPrev.classList.remove(CLASS_CONTROL_HIDE);
                        this._$controlNext.classList.remove(CLASS_CONTROL_HIDE);
                        if (transform === -endTransformValue) this._$controlNext.classList.add(CLASS_CONTROL_HIDE); else if (0 === transform) this._$controlPrev.classList.add(CLASS_CONTROL_HIDE);
                    }
                }
                const activeIndex = [];
                let i = 0;
                ...
                let index;
                let newIndex;
                if ("next" === this._direction) for (i = 0, length = this._activeItems.length; i < length; i++) {
                    index = this._activeItems[i];
                    index += 1;
                    newIndex = index;
                    if (newIndex > this._$itemList.length - 1) newIndex -= this._$itemList.length;
                    activeIndex.push(newIndex);
                } else for (i = 0, length = this._activeItems.length; i < length; i++) {
                    index = this._activeItems[i];
                    index -= 1;
                    newIndex = index;
                    if (newIndex < 0) newIndex += this._$itemList.length;
                    activeIndex.push(newIndex);
                }
            ...
            _moveTo(index) {
                ...
                for (i = 1; i <= Math.abs(diff); i++) this._move();
            }
            _autoplay(action) {
                ...
                if (null === this._intervalId) this._intervalId = setInterval((() => {
                    this._direction = "next";
                    this._move();
            ...
            _refresh() {
                ...
                for (let i = 0, length = $itemList.length; i < length; i++) {
                    ...
                    if (position < itemsInVisibleArea) this._activeItems.push(position);
                }
                this._setActiveClass();
                this._updateIndicators();
                ...
                if (!this._config.loop) {
                    if (this._$controlPrev) this._$controlPrev.classList.add(CLASS_CONTROL_HIDE);
                    return;
                }
                const count = $itemList.length - 1;
                const translate = 100 * -$itemList.length;
        ...
        let mainHomeSlider, clientsMainSlider, clientsReviewsSlider, collaboratorsHomeSlider, ourProjectsSlider;
